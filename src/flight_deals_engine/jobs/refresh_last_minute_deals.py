...
JOB_NAME = "refresh_last_minute_deals"
...
def run(settings: Settings) -> dict[str, Any]:
    ...
    logger.info(
        "Starting job %s for origin=%s destinations=%s horizon_days=%d nights=%d-%d",
        JOB_NAME,
        settings.DEFAULT_ORIGIN,
        settings.LAST_MINUTE_DESTINATIONS,
        settings.LAST_MINUTE_HORIZON_DAYS,
        settings.LAST_MINUTE_NIGHTS_MIN,
        settings.LAST_MINUTE_NIGHTS_MAX,
    )
    ...
    for destination in settings.LAST_MINUTE_DESTINATIONS:
        try:
            logger.debug("Processing destination: %s", destination)
            target = service.build_last_minute_target(settings.DEFAULT_ORIGIN, destination, settings)
            ...
            if best_deal:
                ...
                logger.info("Found last-minute deal for %s: %s", destination, best_deal.price)
            else:
                destinations_empty += 1
                logger.info("No last-minute deals found for %s", destination)

        except (httpx.RequestError, httpx.HTTPStatusError) as e:
            destinations_failed += 1
            logger.error("Failed to fetch last-minute deals for %s: %s", destination, str(e))
            continue
        except Exception:
            destinations_failed += 1
            logger.exception("Unexpected error for destination %s", destination)
            continue

    ranked_deals = service.rank_deals(all_deals)

    if ranked_deals:
        limited_deals = ranked_deals[: settings.LAST_MINUTE_LIMIT]
        logger.info("Writing %d last-minute deals", len(limited_deals))
        writer.write_last_minute_deals(limited_deals)
    else:
        logger.info("No last-minute deals to write.")

    duration_ms = int((time.time() - start_time) * 1000)

    result = {
        "job_name": JOB_NAME,
        "total_destinations": len(settings.LAST_MINUTE_DESTINATIONS),
        ...
        "deals_persisted": len(ranked_deals[: settings.LAST_MINUTE_LIMIT]) if ranked_deals else 0,

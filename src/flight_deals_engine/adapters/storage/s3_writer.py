...
class S3StorageWriter:
    ...
    def write_last_minute_deals(self, items: Sequence[HotDealCandidate]) -> None:
        ...
        file_key = "last_minute_deals.json"
        data = [item.model_dump(mode="json") for item in items]
        ...
        try:
            ...
            logger.info("Saved %d last minute deals to s3://%s/%s", len(items), self.bucket_name, file_key)
        except Exception as e:
            logger.error("Failed to write last minute deals to S3: %s", e)
